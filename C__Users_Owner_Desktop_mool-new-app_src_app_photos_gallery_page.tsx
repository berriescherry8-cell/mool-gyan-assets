...
import { Camera, Expand, ArrowLeft, ChevronLeft, ChevronRight, Loader2, AlertCircle, X } from 'lucide-react';
  ...
  useEffect(() => {
    const staticPhotos: SpiritualPhoto[] = (backendData as any).firestore.structure
      .filter((item:any) => {
        ...
        return item.path.startsWith('/spiritualPhotos/') && (!folder || folder === 'general' || folder === 'Events') && schema?.imageUrl;
      })
    ...
    const sortedPhotos = allPhotos
      .filter(p => p.folder !== 'Pravas aur Prachar-Prasar' && p.imageUrl)
    ...
    <Dialog open={currentIndex !== null} onOpenChange={(isOpen) => !isOpen && setCurrentIndex(null)}>
      <div>
        <div className="flex items-center gap-4 mb-8">
            ...
            <h1 className="text-4xl font-bold font-headline m-0">Photo Gallery</h1>
        </div>
...
export default function GeneralPhotoGalleryPage() {
    ...
    return isClient ? <GeneralPhotoGalleryContent /> : <div className="flex justify-center items-center h-96"><Loader2 className="h-8 w-8 animate-spin" /></div>;
}